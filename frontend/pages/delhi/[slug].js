...
const Home = (props) => {
  ...
  const parentIdOfDelhiPage = "cG9zdDoyMjM2OQ==";

  // Filter the data based on slug and parent ID match
  const currentPageData = props?.data?.graphQl.find((page) => {
    return (
      page.slug === currentPageSlug && page.parentId === parentIdOfDelhiPage
    );
  });
...
export async function getStaticProps({ params }) {
  ...
  try {
    // Fetch pages under the "Delhi" parent page
    const cityParentPagesResponse = await axios.get(
      `https://admin.akclinics.com/wp-json/wp/v2/pages?parent=22369&per_page=20`
    );

    data.cityPages = cityParentPagesResponse.data;

    // Filter the page data for the requested slug from Delhi pages
    ...
    const pagesPerPage = 200; // Adjust this based on your needs
