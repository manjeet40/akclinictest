...
const Home = (props) => {
  ...
  const parentIdOfPage = "cG9zdDoyNTg5Nw==";
...
export async function getStaticPaths({ params }) {
  const response = await fetch(
    "https://admin.akclinics.com/wp-json/wp/v2/pages?parent=22610&per_page=20"
  );
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    // Fetch pages under the "Ludhiana" parent page
    const cityParentPagesResponse = await axios.get(
      `https://admin.akclinics.com/wp-json/wp/v2/pages?parent=25897&per_page=20`
    );

    data.cityPages = cityParentPagesResponse.data;

    // Filter the page data for the requested slug from Ludhiana pages
