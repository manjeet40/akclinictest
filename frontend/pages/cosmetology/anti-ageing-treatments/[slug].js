...
export async function getStaticPaths() {
  const response = await fetch(
    "https://admin.akclinics.com/wp-json/wp/v2/pages?per_page=120"
  );
  const data = await response.json();

  // Filter out pages that have a parent page
  const filteredPaths = data.filter((page) => !page?.parent);
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    // Fetch pages under the "Results" parent page
    const cityParentPagesResponse = await axios.get(
      `https://admin.akclinics.com/wp-json/wp/v2/pages?parent=22880&per_page=20`
    );

    data.cityPages = cityParentPagesResponse.data;

    // Filter the page data for the requested slug from Results pages
