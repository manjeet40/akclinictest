...
const Home = (props) => {
  ...
  const parentIdOfPage = "cG9zdDoyMjI0MA==";
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    // Fetch pages under the "Cosmetology" parent page
    const cityParentPagesResponse = await axios.get(
      `https://admin.akclinics.com/wp-json/wp/v2/pages?parent=22240&per_page=20`
    );

    data.cityPages = cityParentPagesResponse.data;

    // Filter the page data for the requested slug from Cosmetology pages
