...
import HairTransplantCostTable from "@/components/common/hair-transplant-cost-table";
...
import TreatAvailableLudhiana from "@/components/hairTransplantLocation/treatAvailableLudhiana";
...
import CleanWidget from "@/components/common/clean-widget";
...
const Home = (props) => {
  ...
  //   return he.decode(str); // Decode HTML entitie
  // }

  // const title = props?.data?.pageData?.title?.rendered;
  // const decodedTitle = decodeHTMLEntities(title);

  const currentPageSlug = props?.data?.pageData?.slug;
  const currentPageSEOData = props?.data?.graphQl.find((page) => {
    return page.slug === currentPageSlug;
  })?.seo;

  // console.log("Current Page Slug:", currentPageSlug)
  // console.log("Current Page SEO Data:", currentPageSEOData);
  // console.log("All GraphQL Data:", props?.data?.graphQl);
        ...
        {props?.data?.pageData?.acf.sections.map((section, index) => {
          ...
          switch (layout) {
            ...
            case "treatment_available":
              return (
                <TreatAvailableLudhiana props={section.treatment_available} />
              );
            case "results_data":
            ...
            case "hair_transplant_cost_table":
              return (
                <HairTransplantCostTable
                  props={section.hair_transplant_cost_table}
                />
              );
            case "about_journey":
              return <OurJourney props={section.about_journey} />;
            case "clean_widget":
              return <CleanWidget props={section.clean_widget} />;
            case "dr_profile":
...
export async function getStaticPaths() {
  try {
    const response = await fetch(
      "https://admin.akclinics.com/wp-json/wp/v2/pages?per_page=150"
    );
    const data = await response.json();

    // Filter out pages that do not have a parent
    const filteredPaths = data.filter((page) => page.parent === 0);

    // Get the paths of pages without a parent
    const paths = filteredPaths.map((page) => ({
...
export async function getStaticProps({ params }) {
  ...
  try {
    const response = await axios.get(
      `https://admin.akclinics.com/wp-json/wp/v2/pages?slug=${params.slug}&acf_format=standard`
    );

    const filteredResponse = response.data.filter((page) => page.parent === 0);
    if (filteredResponse.length === 0) {
      console.log(
        "No matching page found or page has a parent. Triggering 404..."
      );
      return {
        notFound: true,
      };
    ...
    data.pageData = filteredResponse[0];
    ...
    // Fetch Common Data
    const commonDataResponse = await axios(
      `https://admin.akclinics.com/wp-json/acf/v2/options/`
    );
    data.commonData = commonDataResponse?.data?.acf;
    ...
    while (hasNextPage) {
      // GraphQL query to fetch SEO data for all pages with pagination
